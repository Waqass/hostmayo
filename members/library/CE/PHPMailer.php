    ...
    */
    ...
    function MailSend($header, $body) {
        ...
        for($i = 0; $i < count($this->to); $i++)
        {
            ...
            // check to see if we have a name, and add it to the to
            if ( strlen($this->to[$i][1]) > 0 ) {
    ...
    function EncodeString ($str, $encoding = "base64") {
        ...
        switch(strtolower($encoding)) {
          ...
          case "8bit":
              ...
              if (mb_substr($encoded, -(strlen($this->LE))) != $this->LE)
                $encoded .= $this->LE;
              break;
          case "binary":
              $encoded = $str;
    ...
    function Lang($key) {
        if(count($this->language) < 1)
            $this->SetLanguage("en", 'library/CE/'); // set the default language
        if(isset($this->language[$key]))
            return $this->language[$key];
        else {
            // NEWEDGE MODIFICATION: DON'T RETURN THIS ERROR
