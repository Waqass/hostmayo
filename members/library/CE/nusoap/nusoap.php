...
/*
$Id: nusoap.php,v 1.1.1.1 2006/12/06 20:22:50 alejandro Exp $
...
* @version  $Id: nusoap.php,v 1.1.1.1 2006/12/06 20:22:50 alejandro Exp $
* @access   public
*/
class nusoap_base {
	/**
    ...
    */
	...
	$this->debug("In serializeEnvelope length=" . strlen($body) . " body (max 1000 characters)=" . mb_substr($body, 0, 1000) . " style=$style use=$use encodingStyle=$encodingStyle");
	$this->debug("headers:");
	...
	*/
	function contractQname($qname){
		...
		if (strrpos($qname, ':')) {
			...
			$name = mb_substr($qname, strrpos($qname, ':') + 1);
			// get ns
			$ns = mb_substr($qname, 0, strrpos($qname, ':'));
	...
	function getTypeDef($type){
		...
		if(isset($this->complexTypes[$type])){
			...
			if (!isset($this->simpleTypes[$type]['phpType'])) {
				...
				$uqType = mb_substr($this->simpleTypes[$type]['type'], strrpos($this->simpleTypes[$type]['type'], ':') + 1);
				$ns = mb_substr($this->simpleTypes[$type]['type'], 0, strrpos($this->simpleTypes[$type]['type'], ':'));
			...
			if (!isset($this->elements[$type]['phpType'])) {
				...
				$uqType = mb_substr($this->elements[$type]['type'], strrpos($this->elements[$type]['type'], ':') + 1);
				$ns = mb_substr($this->elements[$type]['type'], 0, strrpos($this->elements[$type]['type'], ':'));
				$etype = $this->getTypeDef($uqType);
				if ($etype) {
		...
		} elseif (mb_ereg('_ContainedType$', $type)) {
...
class soap_transport_http extends nusoap_base {
	...
	*/
	function __construct($url){
		parent::__construct();
		$this->setURL($url);
		mb_ereg('\$Revisio' . 'n: ([^ ]+)', $this->revision, $rev);
...
//		}
		...
		// remove 100 header(s)
		while (mb_ereg('^HTTP/1.1 100',$data)) {
			if ($pos = strpos($data,"\r\n\r\n")) {
	...
	function parseCookie($cookie_str) {
		...
		if ($start > 0) {
			$domain = mb_substr($cookie_str, $start + strlen($cookie_param));
			$domain = mb_substr($domain, 0, strpos($domain, ';'));
		} else {
			$domain = '';
		}
		...
		$start = strpos($cookie_str, $cookie_param);
		if ($start > 0) {
			$expires = mb_substr($cookie_str, $start + strlen($cookie_param));
			$expires = mb_substr($expires, 0, strpos($expires, ';'));
		} else {
			$expires = '';
		}
		...
		$start = strpos($cookie_str, $cookie_param);
		if ( $start > 0 ) {
			$path = mb_substr($cookie_str, $start + strlen($cookie_param));
			$path = mb_substr($path, 0, strpos($path, ';'));
		} else {
			$path = '/';
		}
...
class soap_server extends nusoap_base {
		...
		if(function_exists('getallheaders')){
			...
			foreach ($_SERVER as $k => $v) {
				if (mb_substr($k, 0, 5) == 'HTTP_') {
					$k = str_replace(' ', '-', strtolower(str_replace('_', ' ', mb_substr($k, 5)))); 	                                         $k = strtolower(mb_substr($k, 5));
				} else {
	...
	*/
	function invoke_method() {
		...
		if (strlen($delim) > 0 && mb_substr_count($this->methodname, $delim) == 1 &&
			class_exists(mb_substr($this->methodname, 0, strpos($this->methodname, $delim)))) {
			// get the class and method name
			$class = mb_substr($this->methodname, 0, strpos($this->methodname, $delim));
			$method = mb_substr($this->methodname, strpos($this->methodname, $delim) + strlen($delim));
...
class wsdl extends nusoap_base {
	...
	*/
	function end_element($parser, $name){
		// unset schema status
		if (/*ereg('types$', $name) ||*/ mb_ereg('schema$', $name)) {
			$this->status = "";
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
		...
		if (isset($this->schemas[$ns])) {
			...
			for ($i = 0; $i < count($this->schemas[$ns]); $i++) {
				...
				if ($t) {
					if (!isset($t['phpType'])) {
						// get info for type to tack onto the element
						$uqType = mb_substr($t['type'], strrpos($t['type'], ':') + 1);
						$ns = mb_substr($t['type'], 0, strrpos($t['type'], ':'));
	...
	*/
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		if (strpos($type, ':')) {
			...
			if($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/'){
				...
				if (is_null($value)) {
					if ($use == 'literal') {
						...
						// dpends on nillable, which should be checked before calling this method
			...
			// should the type be compared to types in XSD, and the namespace
	...
	*/
	function __construct($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			if ($pos_xml !== FALSE) {
				$xml_decl = mb_substr($xml, $pos_xml, strpos($xml, '?>', $pos_xml + 2) - $pos_xml + 1);
...
class nusoapclient extends nusoap_base  {
	...
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		$this->debug('SOAP message length=' . strlen($soapmsg) . ' contents (max 1000 bytes)=' . mb_substr($soapmsg, 0, 1000));
		// send
		...
		foreach ($this->operations as $operation => $opData) {
			if ($operation != '') {
				...
				if (sizeof($opData['input']['parts']) > 0) {
					...
					$paramStr = mb_substr($paramStr, 0, strlen($paramStr)-2);
					$paramArrayStr = mb_substr($paramArrayStr, 0, strlen($paramArrayStr)-2);
					$paramCommentStr = mb_substr($paramCommentStr, 0, strlen($paramCommentStr)-2);
				} else {
					$paramStr = '';
