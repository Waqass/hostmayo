...
class UploadHandler
{
    ...
    function __construct($options = null, $error_messages = null) {
        $this->options = array(
            'script_url' => UploadHandler::get_full_url().'/',
    ...
    protected function get_unique_filename($name, $type, $index, $content_range) {
        ...
        //return $this->options['saveasname'];
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file = new stdClass();
        $temp_name = $this->get_file_name($name, $type, $index, $content_range);

        if (strtolower($temp_name) != "blob.png") {
            $file->original_name = $temp_name;
        } else {
            $file->original_name = "upload.png";
        }
        $file->name = $this->options['saveasname'];
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if (!is_dir($upload_dir)) {
                ...
                $html_file = fopen($upload_dir."/index.html", 'w');
                fclose($html_file);
            }
    ...
    public function post($data, $room_id) {
        ...
        foreach ($upload['tmp_name'] as $index => $value) {
            $t_file = $this->handle_file_upload(
            ...
            $files[] = $t_file;
            $data->save_files($room_id,$t_file);
        }
