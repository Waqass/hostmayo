...
/*
$Id: nusoap.php,v 1.4 2007/02/14 20:42:13 mike Exp $
...
* @version  $Id: nusoap.php,v 1.4 2007/02/14 20:42:13 mike Exp $
* @access   public
*/
class lbnusoap_base {
	...
	*/
	...
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$uqType=""){
    	if(is_object($val) && get_class($val) == 'lbsoapval'){
        	return $val->serialize($use);
        }
		$this->debug( "in serialize_val: $val, $name, $type, $name_ns, $type_ns, $attributes, $use");
		// if no name, use item
		$name = (!$name|| is_numeric($name)) ? 'lbsoapVal' : $name;
		...
		switch(true) {
			...
			case (is_array($val) || $type):
                ...
                if($valueType=='arraySimple' || mb_ereg('^ArrayOf',$type)){
					$i = 0;
					if(is_array($val) && count($val)> 0){
						foreach($val as $v){
	                    	if(is_object($v) && get_class($v) ==  'lbsoapval'){
								$tt_ns = $v->type_ns;
								$tt = $v->type;
							} else {
								$tt = gettype($v);
...
* @access   public
*/
function lb_timestamp_to_iso8601($timestamp,$utc=true){
	...
	if($utc){
		...
		if(mb_ereg($eregStr,$datestr,$regs)){
...
* @access   public
*/
function lb_iso8601_to_timestamp($datestr){
	$eregStr =
	'([0-9]{4})-'.	// centuries & years CCYY-
	'([0-9]{2})-'.	// months MM-
	'([0-9]{2})'.	// days DD
...
function lb_usleepWindows($usec)
{
	$start = gettimeofday();
...
* @access public
*/
class lbsoap_fault extends lbnusoap_base {
	...
	*/
	function lbsoap_fault($faultcode,$faultactor='',$faultstring='',$faultdetail=''){
...
* @access   public
*/
class lbXMLSchema extends lbnusoap_base  {
	...
	* @access   public
	*/
	function lbXMLSchema($schema='',$xml=''){
		...
		switch($name){
			...
			case 'complexType':
				if(isset($attrs['name'])){
					...
					if(isset($attrs['base']) && mb_ereg(':Array$',$attrs['base'])){
						$this->complexTypes[$this->currentComplexType]['phpType'] = 'array';
					} else {
						$this->complexTypes[$this->currentComplexType]['phpType'] = 'struct';
					}
	...
	*/
	function expandQname($qname){
		// get element prefix
		if(strpos($qname,':') && !mb_ereg('^http://',$qname)){
			// get unqualified name
...
* @access   public
*/
class lbsoapval extends lbnusoap_base {
	/**
	* constructor
	*
	...
	* @access   public
	*/
  	function lbsoapval($name='lbsoapval',$type=false,$value=-1,$element_ns=false,$type_ns=false,$attributes=false) {
		$this->name = $name;
		$this->value = $value;
		$this->type = $type;
...
* @access public
*/
class lbsoap_transport_http extends lbnusoap_base {
	...
	* constructor
	*/
	function lbsoap_transport_http($url){
		$this->url = $url;
	...
	function getResponse(){
	  ...
	  if ($this->scheme == 'http') {
	    ...
	    while (!isset($lb)){
			...
			// remove 100 header
			if(isset($lb) && mb_ereg('^HTTP/1.1 100',$data)){
				unset($lb);
				$data = '';
			}//
		}
			...
			if(strtolower($this->incoming_headers['content-encoding']) == 'deflate' || strtolower($this->incoming_headers['content-encoding']) == 'gzip'){
    			...
    			if(function_exists('gzuncompress')){
					...
					if($this->incoming_headers['content-encoding'] == 'deflate' && $degzdata = @gzuncompress($data)){
    					$data = $degzdata;
					} elseif($this->incoming_headers['content-encoding'] == 'gzip' && $degzdata = gzinflate(mb_substr($data, 10))){	// do our best
						$data = $degzdata;
					} else {
...
* @access   public
*/
class lbsoap_server extends lbnusoap_base {
	...
	* @access   public
	*/
	function lbsoap_server($wsdl=false){
	...
	function parse_request($data='') {
		...
		if(function_exists('getallheaders')){
			...
			if(strpos($this->headers['Content-Type'],'=')){
				$enc = str_replace('"','',mb_substr(strstr($this->headers["Content-Type"],'='),1));
				if(mb_eregi('^(ISO-8859-1|US-ASCII|UTF-8)$',$enc)){
			...
			foreach ($_SERVER as $k => $v) {
				if (mb_substr($k, 0, 5) == 'HTTP_') {
					$k = str_replace(' ', '-', ucwords(strtolower(str_replace('_', ' ', mb_substr($k, 5)))));
					if ($k == 'Soapaction') {
						// get SOAPAction header
						$k = 'SOAPAction';
						$v = str_replace('"', '', $v);
			...
			if ($this->headers['Content-Encoding'] == 'deflate' || $this->headers['Content-Encoding'] == 'gzip') {
				...
				if (function_exists('gzuncompress')) {
					if ($this->headers['Content-Encoding'] == 'deflate' && $degzdata = @gzuncompress($data)) {
						$data = $degzdata;
					} elseif ($this->headers['Content-Encoding'] == 'gzip' && $degzdata = gzinflate(mb_substr($data, 10))) {
						$data = $degzdata;
					} else {
		...
		$parser = new lbsoap_parser($data,$this->xml_encoding);
	...
	*/
	function fault($faultcode,$faultactor,$faultstring='',$faultdetail=''){
		$this->fault = new lbsoap_fault($faultcode,$faultactor,$faultstring,$faultdetail);
	}
...
* @access public
*/
class lbwsdl extends lbXMLSchema {
    var $wsdl;
    // define internal arrays of bindings, ports, operations, messages, etc.
    var $message = array();
    var $complexTypes = array();
     ...
     * @access public
     */
    function lbwsdl($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false){
        $this->wsdl = $wsdl;
					...
					if ($url != '') {
						...
						if (!isset($urlparts['host'])) {
							$url = $wsdlparts['scheme'] . '://' . $wsdlparts['host'] .
									mb_substr($wsdlparts['path'],0,strrpos($wsdlparts['path'],'/') + 1) .$urlparts['path'];
						}
	                	$this->parseWSDL($url);
					} else {
    ...
    function parseWSDL($wsdl = '')
    {
        ...
        if (isset($wsdl_props['host'])) {
        	...
        	// get wsdl
	        $tr = new lbsoap_transport_http($wsdl);
    ...
    {
        if ($this->status == 'schema' || mb_ereg('schema$', $name)) {
	...
	*/
	function end_element($parser, $name){
		// unset schema status
		if (mb_ereg('types$', $name) || mb_ereg('schema$', $name)) {
			$this->status = "";
		}
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false)
	{
		...
		if ($phpType == 'struct') {
			if (isset($typeDef['element']) && $typeDef['element']) {
				$elementName = $uqType;
				//  use elementFormDefault="qualified|unqualified" to determine
			...
			//  for now, an empty value will be serialized as a zero element
...
* @access   public
*/
class lbsoap_parser extends lbnusoap_base {
	...
	* @access   public
	*/
	function lbsoap_parser($xml,$encoding='UTF-8',$method=''){
		$this->xml = $xml;
	...
	*/
	function start_element($parser, $name, $attrs) {
		...
		foreach($attrs as $key => $value){
			...
			// if ns declarations, add to class level array of valid namespaces
            if($key_prefix == 'xmlns'){
				if(mb_ereg('^http://www.w3.org/[0-9]{4}/XMLSchema$',$value)){
...
* @access   public
*/
class lbsoapclient extends lbnusoap_base  {
	...
	* @access   public
	*/
	function lbsoapclient($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false){
		...
		if($wsdl){
			...
			$this->wsdl = new lbwsdl($this->wsdlFile,$this->proxyhost,$this->proxyport,$this->proxyusername,$this->proxypassword);
	...
	*/
	function send($msg, $soapaction = '', $timeout=0) {
		...
		switch(true){
			// http(s)
			case mb_ereg('^http',$this->endpoint):
				$this->debug('transporting via HTTP');
				if($this->persistentConnection == true && is_object($this->persistentConnection)){
					$http =& $this->persistentConnection;
				} else {
					$http = new lbsoap_transport_http($this->endpoint);
				...
				if(mb_ereg('^http:',$this->endpoint)){
				//if(strpos($this->endpoint,'http:')){
					$this->responseData = $http->send($msg,$timeout);
				} elseif(mb_ereg('^https',$this->endpoint)){
				...
				if($err = $http->getError()){
					...
					if(strpos($http->incoming_headers['content-type'],'=')){
						$enc = str_replace('"','',mb_substr(strstr($http->incoming_headers["content-type"],'='),1));
	...
	*/
    function parseResponse($data) {
		...
		$parser = new lbsoap_parser($data,$this->xml_encoding,$this->operation);
		// if parse errors
