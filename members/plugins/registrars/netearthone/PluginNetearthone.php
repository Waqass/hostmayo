...
class PluginNetearthone extends RegistrarPlugin implements ICanImportDomains
{
    function getVariables()
    {
        $variables = array(
            ...
            lang('Password') => array(
                                'type'          =>'password',
                                'description'   =>lang('Enter the password for your NetEarth One reseller account.'),
                                'value'         =>''
                               ),
            lang('API Key') => array(
                                'type'          =>'text',
    ...
    function checkDomain($params)
    {
        ...
        if ($result == false) {
            ...
            CE_Lib::log(4, 'ERROR: NetEarth One check domain failed.');
            $status = 5;
        }
        $domains = array();
    ...
    function togglePrivacy($params)
    {
        ...
        if ( isset($result->isprivacyprotected) ) {
            ...
            CE_Lib::log(4, 'ERROR: NetEarth One domain details fetch failed with error');
            throw new Exception('Error fetching NetEarth One domain details.');
        }
    }
    ...
    function registerDomain($params)
    {
        ...
        if (is_a($customerId, 'CE_Error')) {
            ...
            throw new CE_Exception('Error creating NetEarth One customer: ' . $customerId->getMessage());
        }
        if ($customerId === false) {
            ...
            if (is_numeric($result)) {
                ...
                CE_Lib::log(4, 'Error creating NetEarth One customer: Unknown Reason');
                throw new Exception('Error creating NetEarth One customer.');
            }
        }
        ...
        if (is_numeric($result)) {
            ...
            CE_Lib::log(4, 'ERROR: NetEarth One customer contact creation failed: Unknown Reason.');
            throw new Exception('Error creating NetEarth One customer contact.');
        }
    ...
    function initiateTransfer($params)
    {
        ...
        if (is_a($customerId, 'CE_Error')) {
            CE_Lib::log(4, 'Error creating NetEarth One customer: ' . $customerId->getMessage());
            throw new Exception('Error creating NetEarth One customer: ' . $customerId->getMessage());
        }
        if ($customerId === false) {
            // Customer doesn't already exist so create one.
            $newCustomer = true;
       ...
       // Handle any extra attributes needed
        ...
        if ($result === false) {
            // Already logged
            throw new Exception('Error transfering NetEarth One domain: A communication problem occurred.');
        }
        CE_Lib::log(2, 'NetEarth One Transfer Result: ' . print_r($result, true));
        ...
        if (isset($result->status) && strtolower($result->status) == 'adminapproved' || strtolower($result->status) == 'success') {
            ...
            CE_Lib::log(4, 'ERROR: NetEarth One domain transfer failed with error: Unknown Reason.');
            throw new Exception('Error transfering NetEarth One domain.');
        }
    }
    ...
    function getContactInformation($params)
    {
        ...
        if (isset($result->status) && $result->status == 'ERROR') {
            ...
            CE_Lib::log(4, 'ERROR: NetEarth One domain registrant contact fetch failed with error');
    ...
    function setContactInformation($params)
    {
        ...
        if (isset($result->actionstatus) && $result->actionstatus == 'Success') {
            CE_Lib::log(4, 'NetEarth One domain contact modified successfully.');
            return $return->actionstatusdesc;
        }
        if (isset($result->status) && $result->status == 'ERROR') {
            CE_Lib::log(4, 'ERROR: NetEarth One error while modifying domain contact details: ' . $result->message);
            throw new Exception('Error modifying domain contact details: ' . $result->message);
        } else {
            CE_Lib::log(4, 'ERROR: NetEarth One error while modifying domain contact details.');
    ...
    function getNameServers($params)
    {
        ...
        if (isset($result->status) && $result->status == 'ERROR') {
            CE_Lib::log(4, 'ERROR: NetEarth One get name servers failed with error: ' . $result->message);
            throw new Exception('Error fetching NetEarth One name servers.: ' . $result->message);
        } else {
            CE_Lib::log(4, 'ERROR: NetEarth One get name servers failed with error');
            throw new Exception('Error fetching NetEarth One name servers.');
        }
    }
    ...
    function setNameServers($params)
    {
        ...
        if (isset($result->status) && $result->status == 'ERROR') {
            if ($result->message == 'Same value for new and old NameServers.') {
                CE_Lib::log(4, 'NetEarth One modify name servers for domain ' . $domain . ' resulted in no changes.');
                return;
            }
            CE_Lib::log(4, 'ERROR: NetEarth One modify name servers failed with error: ' . $result->message);
            throw new Exception('Error during NetEarth One modify name servers command.: ' . $result->message);
        } else {
            CE_Lib::log(4, 'ERROR: NetEarth One modify name servers failed with error');
            throw new Exception('Error during NetEarth One modify name servers command.');
        }
        CE_Lib::log(4, 'NetEarth One modify name servers for domain ' . $domain . ' has been completed successfully.');
    }
    ...
    function registerNS($params)
    {
        ...
        if (isset($result->actionstatus) && $result->actionstatus == 'Success') {
            CE_Lib::log(4, 'NetEarth One addition of child name server ' . $params['nsname'] . ' successful.');
            return $result->actiontypedesc;
        }
        if (isset($result->status) && $result->status == 'ERROR') {
            CE_Lib::log(4, 'ERROR: NetEarth One add child name servers failed with error: ' . $result->message);
            throw new Exception('Error during NetEarth One add child name servers command.: ' . $result->message);
        } else {
            CE_Lib::log(4, 'ERROR: NetEarth One add child name servers failed with an error.');
            throw new Exception('Error during NetEarth One add child name servers command.');
        }
    }
    ...
    function editNS($params)
    {
        ...
        if (isset($result->status) && $result->status == 'Success') {
            CE_Lib::log(4, 'NetEarth One modification of child name server ' . $params['nsname'] . ' successful.');
            return $result->actiontypedesc;
        }
        if (isset($result->status) && $result->status == 'ERROR') {
            CE_Lib::log(4, 'ERROR: NetEarth One modify child name servers failed with error: ' . $result->message);
            throw new Exception('Error during NetEarth One modify child name servers command.: ' . $result->message);
        } else {
            CE_Lib::log(4, 'ERROR: NetEarth One modify child name servers failed with an error.');
            throw new Exception('Error during NetEarth One modify child name servers command.');
        }
    }
    ...
    function _lookupCustomerId($email)
    {
        ...
        if (isset($result->customerid) && $result->customerid > 0) {
            CE_Lib::log(4, 'NetEarth One customer "' . $email . '" already exists: ' . $result->customerid);
            return $result->customerid;
        }
        CE_Lib::log(4, 'NetEarth One customer "' . $email . '" does not already exist.');
        return false;
    }
    ...
    function _lookupDomainId($domain)
    {
        ...
        if (is_numeric($result)) {
            CE_Lib::log(4, 'NetEarth One domain id "' . $result . '" found for domain ' . $domain . '.');
            return $result;
        }
        if (isset($result->status) && $result->status == 'ERROR') {
            CE_Lib::log(4, 'ERROR: NetEarth One error occurred while looking up domain id for ' . $domain . '.  Error: ' . $result->message);
            throw new Exception('An error occurred while connecting to NetEarth One.  Error: ' . $result->message);
        }
        CE_Lib::log(4, 'ERROR: NetEarth One error occurred while looking up domain id for ' . $domain . '.  Error: Unknown Error.');
        throw new Exception('An error occurred while connecting to NetEarth One.  Error: Unknown');
    }
