...
class PluginResellerclub extends RegistrarPlugin implements ICanImportDomains
{
    ...
    private function searchHosts($params, $type)
    {
        ...
        if ($result === false) {
            throw new Exception('A connection issued occurred while connecting to ResellerClub.', EXCEPTION_CODE_CONNECTION_ISSUE);
        } else if (strtolower($result->status) == 'error' && $result->message == 'You need to activate your FREE DNS Services before you can perform this action') {
    ...
    function _lookupDomainId($domain)
    {
        ...
        if (isset($result->status) && $result->status == 'ERROR') {
            ...
            } else if (isset($result->message) && strpos($result->message, "Website doesn't exist for") !== false) {
