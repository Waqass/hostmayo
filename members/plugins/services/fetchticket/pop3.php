 ...
 * pop3.php
 *
 * @(#) $Header: /home/cvs/clientexec/plugins/services/fetchticket/pop3.php,v 1.1 2006/03/13 21:04:19 alejandro Exp $
 *
 */
...
class pop3_class
{
	...
	{
    CE_Lib::debug("pop3_class: " . $message);
	}
	...
	Function Login($user,$password,$apop=0)
	{
		...
		else
		{
			...
			if(!$authenticated)
			{
				...
				if($this->Tokenize($response," ")!="+OK")
					return($this->SetError("Password error for user '$user' : ".$this->Tokenize("\r\n")));
			}
		}
		...
		for($headers=$body=array(),$line=0;;)
		{
			...
			switch($response)
			{
				...
				case "":
					break 2;
				default:
					if(mb_substr($response,0,1)==".")
						$response=mb_substr($response,1,strlen($response)-1);
					break;
			}
