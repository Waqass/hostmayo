...
CE.prototype.getGravatarUrl = function(gravataremail,size,setdefault)
{
    ...
    var gravUrl = "", defaultpicurl, pathArray, newPathname = "";
    if (!size) size = 80;
    ...
    if (gravataremail && gravataremail!="") {
        ...
        defaultpicurl = window.location.protocol + "//" + window.location.host + "/" + newPathname + "/../templates/common/img/avatars/avatar_male_gray_on_dark_200x200.png";
        gravUrl = "https://secure.gravatar.com/avatar.php?gravatar_id="+MD5(gravataremail)+"&size="+size+"&rating=PG";
        if (setdefault) gravUrl += "&default=" + encodeURIComponent(defaultpicurl);
    }
    return gravUrl;
};
  ...
  var newInput = {};
    switch (inputObj.type) {
        ...
        case 'password':
            ...
            newLabel = '<label data-toggle="tooltip" title="'+inputObj.description+'" for="input'+inputObj.name+'">'+inputObj.label+'</label>';
            break;
        case 'textarea':
            newInput = $('<textarea rows="8" id="textarea'+inputObj.name+'" name="'+inputObj.name+'"></textarea>');
            newLabel = '<label data-toggle="tooltip" title="'+inputObj.description+'" for="textarea'+inputObj.name+'">'+inputObj.label+'</label>';
            break;
        case 'checkbox':
        case 'yesno':
                newInput = $('<select id="select'+inputObj.name+'" name="'+inputObj.name+'"><option value="1">Yes</option><option value="0">No</option></select>');
                newLabel = '<label data-toggle="tooltip" title="'+inputObj.description+'" for="select'+inputObj.name+'">'+inputObj.label+'</label>';
                break;
        default:
            return false;
        }
